...
if (isset($_GET['edit'])) {
		$id = $_GET['edit'];
		$edit = mysqli_query($koneksi, "SELECT * FROM m_users WHERE id ='$id'");
...
if (isset($_POST['ubah'])) {
	...
	$update = mysqli_query($koneksi, "UPDATE m_users SET email='$email',password='$password' WHERE id = '$id'");		
	
	if($update){
		header("location:?pg=User&ubah=berhasil");
	}
	else{
		header("Location:?pg=ubah-user&edit=".$id."&ubah=gagal");
		...
		<form action="" method="post" class="mt-3 pl-2">
			...
			<div class="form-group">
				<div class="row">
					...
					<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10">
						<input type="text" name="email" placeholder="email" value="<?php echo $rowEdit['email'] ?>">
					</div>
				</div>
			</div>					
			...
			<div class="form-group">
				...
				<div class="row">
					...
					<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10">
						<input type="password" class="form-control" name="password" value="<?php echo $rowEdit['password'] ?>">
					</div>
				</div>
			</div>
